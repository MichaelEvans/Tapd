...
public class GlassConfigActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Define our layout
        setContentView(R.layout.activity_glass_config);
        ...
        if (savedInstanceState != null) {
            ...
            Editor prefs = getSharedPreferences("glass", Context.MODE_PRIVATE).edit();
            prefs.putString("glass", PARAM_AUTH_TOKEN);
            prefs.commit();
            Log.e("Glass", PARAM_AUTH_TOKEN);
        } else {
        ...
        mExpireTokenButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (!TextUtils.isEmpty(mAuthToken)) {
                    ...
                    GoogleAuthUtil.invalidateToken(GlassConfigActivity.this, mAuthToken);
                    mAuthToken = null;
                    mExpireTokenButton.setEnabled(false);
                    mStartAuthButton.setEnabled(true);
                }
                ...
                try {
                    ...
                    client.createTimelineItem(mAuthToken, json, new MirrorApiClient.Callback() {
                        ...
                        public void onSuccess(HttpResponse response) {
                            ...
                            Toast.makeText(GlassConfigActivity.this, "Created new timeline item",
                        ...
                        public void onFailure(HttpResponse response, Throwable e) {
                            ...
                            Toast.makeText(GlassConfigActivity.this, "Failed to create new timeline item",
                                    Toast.LENGTH_SHORT).show();
                        }
                    });
    ...
    private void onTokenResult(String token) {
        ...
        if (!TextUtils.isEmpty(token)) {
            ...
            Editor prefs = getSharedPreferences("glass", Context.MODE_PRIVATE).edit();
            prefs.putString("glass", mAuthToken);
            prefs.commit();
            Log.e("Glass", mAuthToken);
        } else {
            mExpireTokenButton.setEnabled(false);
            mStartAuthButton.setEnabled(true);
            ...
            public void run() {
                try {
                    ...
                    final String token = GoogleAuthUtil.getToken(
                    		GlassConfigActivity.this, account, SCOPE);
                    ...
                    mHandler.post(new Runnable() {
                        @Override
                        public void run() {
                            Toast.makeText(GlassConfigActivity.this,
